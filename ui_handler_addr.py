...
class JPDataUIHandlerAddr:
    ...
    def _connect_signals(self):
        ...
        self.mesh3_action.triggered.connect(self.add_mesh3_from_selected)
    ...
    def add_graticule_layer(self, interval=10):
        ...
        for lon in range(-180, 181, interval):
            points = [QgsPointXY(lon, lat) for lat in range(-90, 91)]
        ...
        for lat in range(-80, 81, interval):
            points = [QgsPointXY(lon, lat) for lon in range(-180, 181)]
            feat = QgsFeature(layer.fields())
            feat.setGeometry(QgsGeometry.fromPolylineXY(points))
            feat["type"] = "latitude"
            feat["value"] = lat
